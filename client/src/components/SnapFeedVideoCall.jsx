...
export default function SnapFeedVideoCall({ socket, currentUserId, targetUser, onEndCall }) {
    ...
    try {
      ...
      pc.onicecandidate = (event) => {
        if (event.candidate && socket) {
          socket.emit('ice_candidate', { to: targetUser._id, candidate: event.candidate });
        }
      };
      ...
      if (socket) {
        socket.emit('call_offer', { to: targetUser._id, offer, from: currentUserId });
      }
  ...
  const endCall = () => {
    ...
    if (socket) {
      socket.emit('call_end', { to: targetUser._id });
      ...
      <div className="absolute top-0 left-0 right-0 p-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-indigo-500 flex items-center justify-center text-sm font-bold text-white overflow-hidden">
            {targetUser.avatar ? <img src={targetUser.avatar} className="w-full h-full object-cover" alt="" /> : targetUser.fullName?.charAt(0)}
          </div>
          <div>
